<div class="nav-container">
    <div class="mobile-topbar-header">
        <div class="">
            <img src="assets/images/logo-icon.png" class="logo-icon-2" alt="" />
        </div>
        <div>
            <h4 class="logo-text">Supedia</h4>
        </div>
        <a href="javascript:;" class="toggle-btn ml-auto"> <i class="bx bx-menu"></i>
        </a>
    </div>
    <nav class="topbar-nav">
        <ul class="metismenu" id="menu">
            <li>
                <a href='/admin_area/admin.php?tn="s"' class="has-arrow">
                    <div class="parent-icon icon-color-1"><i class="bx bx-home-alt"></i>
                    </div>
                    <div class="menu-title">Add product</div>
                </a>
            </li>
            <li>
                <a href="/admin_area/productlist.php" class="has-arrow">
                    <div class="parent-icon icon-color-2"><i class='bx bx-briefcase-alt'></i>
                    </div>
                    <div class="menu-title">Product list</div>
                </a>
            </li>
            <li>
                <a class="has-arrow" href="../assets/php/errors-coming-soon.php">
                    <div class="parent-icon icon-color-1"><i class="bx bx-line-chart"></i>
                    </div>
                    <div class="menu-title">Add Category</div>
                </a>

            </li>


            <li>
                <a class="has-arrow" href="../assets/php/errors-coming-soon.php">
                    <div class="parent-icon icon-color-2"> <i class="bx bx-donate-blood"></i>
                    </div>
                    <div class="menu-title">Add Brand</div>
                </a>

            </li>

        </ul>
    </nav>
</div>